...
inline bool broadcast_merge(InferenceContext* c, DimensionHandle a, DimensionHandle b, DimensionHandle& out)
{
...
Status BroadcastFMAOpOutputShapeImpl(InferenceContext* c, int n) {
  ...
  for(int i=1; i<n; i++) {
    ...
    for(int i=new_rank-1; i>=0; i--) {
        int ii = new_rank-1-i;
        DimensionHandle d1 = ii<rank_out 
            ? c->Dim(out, rank_out-1-ii) 
            : c->MakeDim(1);
        DimensionHandle d2 = ii<rank_s
            ? c->Dim(s, rank_s-1-ii) 
            : c->MakeDim(1);
        bool ok = broadcast_merge(c, d1, d2, new_dims[i]);
        if(!ok) 
            return errors::InvalidArgument("FMA incompatible shapes");
    }
